# Adopt JUnit and Mockito as Standard Testing Framework Stack: Integration Tests Test
...
- **R-JUNIT-MOCKITO-001** SHOULD: Integration tests SHOULD test complete workflows across multiple components or layers.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Automated CI pipeline checks MUST verify JUnit and Mockito are the only testing frameworks in use. Code review checklists MUST include verification of test naming conventions and framework usage. CI builds MUST fail if tests are found that do not follow naming conventions (*Test.java or *IT.java).
</enforcement>