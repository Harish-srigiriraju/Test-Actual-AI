# Adopt JUnit and Mockito as Standard Testing Framework Stack: Unit Tests Use
...
- **R-JUNIT-001** MUST: All unit tests MUST use JUnit as the primary testing framework.
- **R-JUNIT-002** MUST: All test files MUST follow naming conventions: unit tests end with `Test.java` and integration tests end with `IT.java`.
- **R-JUNIT-003** MUST: Mock objects MUST be created using the org.mockito framework.
- **R-JUNIT-004** SHOULD: Test methods should clearly describe the scenario being tested (e.g., `shouldReturnUserWhenValidIdProvided`).
- **R-JUNIT-005** SHOULD: Maintain a balanced test pyramid with appropriate mix of unit tests (with mocks) and integration tests (with real dependencies).
